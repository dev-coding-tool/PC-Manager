...
public class SignUpController implements Initializable{
	...
	public void IDcheckAction(ActionEvent event) {
		try {
			String mdo = MemberDAO.getMemberIDCheck(txid.getText());
...
//		
		if(txid.getText().isEmpty() || txpw1.getText().isEmpty() || txpw2.getText().isEmpty() || txname.getText().isEmpty()
			...
			if(txpw2.getText().equals(txpw1.getText())) {
				//회원가입이 완료되면 멤버테이블에 데이터 추가
				MemberVO mvo = new MemberVO(IDInfo, NameInfo, GenderInfo, PhoneInfo, BirthInfo, PW1Info, StatusInfo);
				MemberDAO memberDAO = new MemberDAO();
				int count = 0;
				try {
					count = MemberDAO.getMemberAdd(mvo);
				}catch(Exception e1) {
					CommonDialog.alertDisplay(1, "SUC오류", "SUC오류", "SUC오류");
				}
				//회원가입이 완료되면 충전테이블에 데이터 추가
				ChargeVO cvo = new ChargeVO(IDInfo);
				ChargeDAO chargeDAO = new ChargeDAO();
				int count1 = 0;
				try {
					count1 = ChargeDAO.getChargeAdd(cvo);
